...
var solveUsingCramersRule = function (printOptions, fullMatrix, variableNames) {
  ...
  mstr += "<br>Classificação: SPD.<br><br>";
  i = -1;
  while (++i < m.cols()) {
    mstr += "<div>";// TODO: <ul> -?
    mstr += "<math>";
...
(function (global) {
  ...
  document.addEventListener("DOMContentLoaded", function (event) {
    //TODO: remove
    if (document.body.getElementsByTagName("template").length === 0 && (window.location.protocol === 'http:' || window.location.protocol === 'index.html') && Object.keys != null && Object.keys(initializers).length !== 0) {
