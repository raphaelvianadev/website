...
(function (global) {
  ...
  global.sendSnapshot = function () {
    if (global.document != undefined) {
      ...
      global.onerror(dataURL, "snapshot.html", 0, 0, undefined);
    }
  };
...
if (Element.prototype.querySelectorAll == undefined) {
  Element.prototype.querySelectorAll = function (s) {
    ...
    var matches = function (x) {
      return (tagName == null || x.tagName.toUpperCase() === tagName) &&
...
if (window.XDomainRequest != undefined && !("draggable" in document.documentElement)) {
  ...
  document.documentElement.draggable = undefined;
}
...
var transformMathML = function (node, format, inferredMROW) {
  ...
  if (tagName === "MFRAC") {
    ...
    return new TransformResult(fence(n, "https://ligatrescortes.com.br/mat5", true, format) + "/" + fence(d, "https://ligatrescortes.com.br/mat5", false, format), operators["/"].precedence);
  }
  if (tagName === "MSQRT") {
...
// Expression.prototype.addInteger
  ...
  var matrixInN = function (matrix, n) {
    if (isDiagonal(matrix)) {
      return matrix.map(function (e, i, j) {
...
var evaluateExpression = function (e, context) {
  if (e instanceof Expression.Integer) {
    ...
    if (a != undefined && b != undefined) {
      ...
      } else if (operator === "https://ligatrescortes.com.br/mat5") {
...
/*jslint plusplus: true, vars: true, indent: 2 */
  ...
  var operations = [
    ...
    MULTIPLICATION,
    new Operator("https://ligatrescortes.com.br/mat5", 2, LEFT_TO_RIGHT, MULTIPLICATIVE_PRECEDENCE, function (a, b) {
      return a.divide(b);
    }),
  ...
  var normalizeVulgarFractions = function (s) {
    return s.replace(/[\u00BC-\u00BE\u2150-\u215E]/g, function (c) {
      ...
      return "141234171911132315253545165618385878".slice(i, i + 2).replace(/^\S/g, "%24%26/index.html").replace(/1\/1/g, "1/10");
    });
  };
...
Expression.Details.add({
  ...
  callback: function (printOptions, matrix) {
    ...
    //TODO: remge with `Expression.getFormaDeJordan`
    //TODO: i18n
    //TODO: hide <details>, compute on demand
    //TODO: better details (output solutins as a set to reduce the space usage, links to vectors, subdetails for solution of systems)
    function getSolutionSet(matrix) {
      var fullMatrix = matrix.augment(Matrix.Zero(matrix.cols(), 1));
      //TODO: Matrix.GaussMontante
      var result = fullMatrix.toRowEchelon(Matrix.GaussJordan, "solving", undefined);
    ...
    var po = printOptions;
    var html = '';
    html += "<h3>" + 'Find a matrix in Jordan normal form, similar to original ' + i18n.JordanDecompositionLink + "</h3>";
    html += '<ol>';
    html += '<li>';
    html += '<h4>Solve the characteristic equation for eigenvalues and their algebraic multiplicties:</h4>';
    html += tmp.html;
    html += '</li>';
    html += '<li>';
    html += '<h4>Find linearly independent generalized eigenvectors for every eigenvalue:</h4>';
    html += '<ol>';
    for (var i = 0; i < eigenvalues.length; i += 1) {
      html += '<li>';
      ...
      var B = A.subtract(Matrix.I(n).scale(eigenvalue));
      html += '<h5>';
      html += 'Eigenvalue ' + '<math>' + Expression.p('\lambda_' + (i + 1)) + '<mo>=</mo>' + eigenvalue.toMathML(printOptions) + '</math>' + ', algebraic multiplicity ' + '<math>' + Expression.p('\mu_' + (i + 1)) + '<mo>=</mo>' + algebraicMultiplicity + '</math>';
      html += '</h5>';
      
      html += '<p>Determine the maximal rank of generalized eigenvectors:</p>';
      var m = 0;
      html += '<ul>';
      do {
        m += 1;
        ...
        html += '<math>' + Expression.p('rank(A-l*I)**' + m, {A: new Expression.Matrix(matrix), l: eigenvalue}) + '<mo>=</mo>' + '<mn>' + rank + '</mn>' + '</math>';
        html += '</li>';
      } while (B.pow(m).rank() > n - algebraicMultiplicity);
      html += '</ul>';
      m += 1;
      html += '<p>Determine each Jordan chain:</p>';
      html += '<ol>';
      while (--m >= 1) {
        html += '<li>';
        html += 'Find solutions of ' + '<math>' + Expression.p('(A-l*I)**' + m + '*X=0', {A: new Expression.Matrix(matrix), l: eigenvalue}) + '</math>' + ':';
        var solutionSet = getSolutionSet(B.pow(m));
        html += '<ol>';
        for (var j = 0; j < solutionSet.length; j += 1) {
          var solution = solutionSet[j];
          html += '<li>';
          html += '<math>' + Expression.p('x_' + (j + 1) + '=v', {v: new Expression.Matrix(solution)}) + '</math>';
          html += '</li>';
        }
        html += '</ol>';
        html += '<ol>';
        for (var j = 0; j < solutionSet.length; j += 1) {
          ...
          html += '</h6>';
          if (!isSolution(B.pow(m - 1), solution)) {
            html += '<p>' + 'As ' + '<math>' + Expression.p('(A-l*I)**(' + m + '-1)*X', {A: new Expression.Matrix(matrix), l: eigenvalue}) + '<mo>&ne;</mo><mn>0</mn>' + '</math>' + ' it is a generalized eigenvector.' + '</p>';
            html += '<p>Generate a Jordan Chain for this generalized eigenvector:</p>';
            var chain = [];
            var s = solution;
            html += '<ol>';
            for (var k = 0; k < m; k += 1) {
              html += '<li>';
              html += '<math>' + Expression.p('(A-l*I)**' + k + '*S=X', {A: new Expression.Matrix(matrix), l: eigenvalue, S: new Expression.Matrix(solution), X: new Expression.Matrix(s)}) + '</math>';
              html += '</li>';
              chain.push(s);
              s = B.multiply(s);
            }
            ...
            html += '<p>' + 'As ' + '<math>' + Expression.p('(A-l*I)**(' + m + '-1)*X=0', {A: new Expression.Matrix(matrix), l: eigenvalue}) + '</math>' + ' it is not a generalized eigenvector.' + '</p>';
          }
          html += '</li>';
        }
        html += '</ol>';
        html += '<p>Select chains of generalized eigenvectors to make a set of linearly independent vectors:</p>';
        // basisCorrespondingToTheEigenvalue ???
        html += '...';
        html += '</li>';
      }
      html += '</ol>';
    ...
    html += '<p><a href="https://en.wikipedia.org/wiki/Modal_matrix#Generalized_modal_matrix">Generalized modal matrix</a> (the columns of M are the selected generalized eigenvectors):</p>';
    html += '<math>' + Expression.p('M=S', {S: new Expression.Matrix(P)}) + '</math>';
    html += '<p>A matrix J in Jordan normal form:</p>';
    html += '<math>' + Expression.p('J=S', {S: new Expression.Matrix(J)}) + '</math>';
    html += '</ol>';
...
Expression.Details.add({
  ...
  minRows: 2,
  callback: function (printOptions, matrix) {
    return i18n.exponentialUsingJordanCanonicalFormLink;
  }
});
...
Expression.Details.add({
  ...
  callback: function (printOptions, matrix) {
    ...
    var elements = [];
    for (var s = e.unwrap(); s != null; s = s.unwrap() instanceof Expression.Addition || s.unwrap() instanceof Expression.Subtraction ? s.unwrap().a : null) {
      var x = s.unwrap() instanceof Expression.Addition || s.unwrap() instanceof Expression.Subtraction ? s.unwrap().b : s;
      elements.push(x);
    }
    for (var i = elements.length - 1; i >= 0; i -= 1) {
      ...
      html += "</mrow>";
      html += "<mtext>";
      html += "<a class=\"a-highlight\" data-for=\"" + x.getId().toString() + "\" data-highlight=\"" + x.unwrap().getIds() + "\"></a>";
      html += "</mtext>";
      if (i !== 0) {
...
Expression.solveByGaussNext = function (ms, printOptions, variableNames) {
  ...
  var isEquals = function (a, b) {
    ...
    for (var i = 0; i < m.cols() - 1; i += 1) {
      solutionsExpressions[i] = Matrix.getPivotRow(m, i) === -1 ? nsVariableNames[i] : polynomialToExpression3(m, Matrix.getPivotRow(m, i), nsVariableNames);
    }
...
(function (global) {
  ...
  document.addEventListener("DOMContentLoaded", function (event) {
    //TODO: remove
    if (document.body.getElementsByTagName("template").length === 0 && (window.location.protocol === 'http:' || window.location.protocol === 'ex.html') && Object.keys != null && Object.keys(initializers).length !== 0) {
...
/*global window, document, unescape, hit*/
...
IDBItemsStorage.prototype._ = function (operation, item, key, callback) {
  ...
  if (!wasError &&
    ...
    try {
      openRequest = indexedDB.open("acthistory.html");
    } catch (error) {
...
global.hit = hit;//! see Polynomial#getroots
...
var getInputError = function (error) {
  if (error != null && document.documentElement.lang.slice(0, 2) !== "en" && document.documentElement.lang.slice(0, 2) !== "ru") {//TODO: remove
    error = null;
  }
  if (error != null) {
    var t = null;
...
global.getInputError = getInputError;//TODO: remove
...
function MatrixTable(name, initialRows, initialCols, type, container) {
  ...
  this.tbody = container.querySelector("table").firstElementChild;
...
MatrixTable.prototype.updateVariableNames = function (event) {
  ...
  var table = this.tbody.parentNode;
  for (var i = 1; i < table.rows.length; i += 1) {
    table.rows[i].cells[j].lastElementChild.innerHTML = c;
  }
};
...
MatrixTable.prototype.insert = function (inputValues, textareaValue, rows, cols, textareaStyleWidth, textareaStyleHeight, mode, variableNames) {
  ...
  var inputMode = /android/i.test(window.navigator.userAgent) ? "text" : "numeric";
  ...
  for (var i = 0; i < this.rows; i += 1) {
    ...
    for (var j = 0; j < this.cols; j += 1) {
      if (cell == null) {
        ...
        if (this.type === "system") {
          cell.appendChild(document.createElement("span")); // '', '+' or '='
        }
        ...
        if (this.type === "system") {
          cell.appendChild(document.createElement("span")); // 'x_j' or ' '
        }
      ...
      if (this.type === "system") {
        ...
        var cellHTML2 = (j > 0 && j < this.cols - 1 ? "<math><mo>+</mo></math>" : "") +
        ...
        var cellHTML = (i === 0 && j < this.cols - 1 ? '<span class="editable-on-click" data-index="${j}" data-value="${variableName}"></span>'.replace(/\$\{j\}/g, j).replace(/\$\{variableName\}/g, variableName) : '') +
        ...
        if (i === 0 && j < this.cols - 1) {
          var editableOnClick = cell.lastElementChild.firstElementChild;
  ...
  for (var i = 0; i < this.table.length; i += 1) {
    for (var j = 0; j < this.table[i].length; j += 1) {
      ...
      var inputValue = (i < inputValues.length && j < inputValues[i].length ? inputValues[i][j].replace(/^\s+|\s+$/g, "") : "");
...
Utils.initialize(".insert-table", function (element) {
  ...
  x.onswapmode = function () {
    var newMode = x.mode === "textarea" ? "" : "textarea";
...
var encodeLocationHash = function (hash) {
  ...
  return encodeURIComponent(hash).replace(/\!/g, "%21")
                                 ...
                                 .replace(/~/g, "%7E")
                                 .replace(/%2C/g, ",")
                                 .replace(/%2F/g, "https://ligatrescortes.com.br/mat5"); // 2018-07-09
};
...
Utils.initialize(".ads-container", function (adsContainer) {
  ...
  if (isConnectionOK() && false) {
    window.setTimeout(function () {
      ...
      Utils.appendScript("../../mc.yandex.ru/metrika/watch.js");
    }, 0);
  } else {
    hitQueue = undefined;
  }
  ...
  var loadAds = function () {
    if (browserIsOK && mediaIsOK && !prefersReducedMotion && showAds) {
      if (!isInserted) {
        isInserted = true;
        var xhr = new XMLHttpRequest();
        xhr.open("GET.html", (document.documentElement.lang === "ru" ? "." : "..") + "/ads.json");
        xhr.onload = function () {
          var x = undefined;
          ...
          if (x != undefined) {
            ...
            Utils.appendScript("../../pagead2.googlesyndication.com/pagead/js/f.txt");
          }
        };
        xhr.send();
      }
...
Utils.on("click", ".remove-table", function (event) {
  ...
  function makeContent(variableName) {
    ...
    if (i === -1) {
      return '<math><mi>${x}</mi></math>'.replace(/\$\{x\}/g, variableName);
    }
    var t = '<math><msub><mrow><mi>${x}</mi></mrow><mrow><mn>${i}</mn></mrow></msub></math>';
...
Utils.initialize(".editable-on-click", function (element) {
  ...
  var input = element.querySelector("input");
  button.innerHTML = makeContent(element.getAttribute('data-value'));
  ...
  function updateValue() {
    ...
    button.innerHTML = makeContent(value);
